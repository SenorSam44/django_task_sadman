...
from .serializers import (
    ConnectSerializer, BookingSystemSerializer, ProviderSerializer,
    CustomerSerializer, ServiceSerializer, AppointmentSerializer
)
...
class ConnectView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(BookingSystemSerializer(bs).data, status=status.HTTP_201_CREATED)
...
class ServiceListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        bs = get_object_or_404(BookingSystem, id=self.kwargs['id'])
        return bs.services.all().order_by('id')  # Added order_by to fix unordered queryset warning
